...
select distinct city, avg(cnt) as avg_order_num, avg(far) as avg_income
from (select driver_id, city, count(order_id) as cnt, sum(fare) as far
      from tb_get_car_order, tb_get_car_record
      where tb_get_car_order inner join tb_get_car_record on tb_get_car_order.order_id = tb_get_car_record.order_id
          and city = '北京'
          and order_time >= '2021-10-01 00:00:00' and order_time <= '2021-10-07 23:59:59'
      group by driver_id
      having count(order_id) >= 3);


select driver_id, city, count(order_id) as cnt, sum(fare) as far
from tb_get_car_order, tb_get_car_record
where tb_get_car_order inner join tb_get_car_order on order_id
and city = '北京'
and order_time >= '2021-10-01 00:00:00' and order_time <= '2021-10-07 23:59:59'
group by driver_id
having count(order_id) >= 3;
